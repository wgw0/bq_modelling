...
def vectorize_features(X_agg_dicts, X_seq_dicts, max_seq_length):
    ...
    X_agg = X_agg_sparse.toarray().astype(np.float32)  # Convert to float32 for speed

    X_seq = []
    for seq in X_seq_dicts:
        seq_sparse = vec.transform(seq)
        seq_vec = seq_sparse.toarray().astype(np.float32)  # Convert to float32 for speed / change back for full implementation
