...
# -----------------------------------------------------------------------------
# Configuration and Hyperparameters (values can be overridden via environment variables)
# -----------------------------------------------------------------------------
...
RANDOM_STATE = 12
BQ_DATE_RANGE = (
    os.environ.get("BQ_DATE_RANGE_START", "20241001"),
    os.environ.get("BQ_DATE_RANGE_END", "20250228")
)

np.random.seed(RANDOM_STATE)
tf.random.set_seed(RANDOM_STATE)

# Generate a unique model id and set paths to save artifacts.
unique_model_id = uuid.uuid4().hex[:8]
model_dir = "models"
if not os.path.exists(model_dir):
    os.makedirs(model_dir)
MODEL_CHECKPOINT_PATH = os.path.join(model_dir, f"model_{unique_model_id}.h5")
VECTORIZER_PATH = os.path.join(model_dir, f"vectorizer_{unique_model_id}.pkl")
LABEL_ENCODER_PATH = os.path.join(model_dir, f"label_encoder_{unique_model_id}.pkl")
...
def encode_labels(y_labels):
    """Encode string labels into one-hot vectors."""
...
def main():
    ...
    end_time = time.time()
    logging.info(f"Total training time (seconds): {end_time - start_time:.2f}")
